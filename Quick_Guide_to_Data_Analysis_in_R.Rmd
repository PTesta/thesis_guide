...
In general, it's a good idea to compile often (as a way of checking for errors in your code). You do this by clicking the "knit" button in the script window (top left) or by clicking command+shift+k (cntrl+shift+k on a PC).
...
There are lots of ways to look at the data. If we were interested in the education and party ID of the first ten respodents we could type
...
Different surveys have different conventions for how to label these responses. The NES typically uses 998 or 999. Some other surveys will leave them blank or code them as something else. For our purposes, we want to tell R to treat these data as "missing" or "not avaialbe". 

1. First we'll create a new variable, called "ft_trump" that's just a copy of the original varialbe
2. Next, we'll set the values of this new variable that equal 998 to NA (where NA is R's way of knowing the data are "not availabe")
...
Note the different outputs of the summary function (which gives you the mininum 25th percentile, median [50th percentile], mean, 75th percentile, and maximum)
...
The test stastic is slightly different from the t-test because of some assumptions about equal or unequal variances in these two groups, which we won't concern ourselves with, but with some adjustments, the linear model can produce the same results
...
Recall, racial identity is an ordinal variable. OLS treats it's as continous variable with interval properties (the change from 0 to 1 is the same from 3 to 4). For now we'll ignore this somewhat strong assumption and focus on the result, which is a weakly positive replatiship between feelings and racial identity. Specifically, someone who says their racial identity is not at all important, has generally cool feeling toward Trump 35.54. Someone who feels very strongly about their racial identity has a warmer feeling toward Trump (35.54+1.29$\times$4= `r 35.54+1.29*4`)
...
Let's fit the following mutliple regression models to test these questions:
...
To do that, we'll create a prediction dataframe
